...
const schema=require('../models/information_tbls');
...
router.get('/:emailid/:password',function(req,res,next){
    schema.find({"u_email":req.params.emailid,"u_password":req.params.password})
    .then(doc=>{
        if(doc.length>0)
        {
            res.json(doc);
